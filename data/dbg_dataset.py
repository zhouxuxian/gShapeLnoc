...
import gc
from models.utils import de_Bruijn_graph, get_freq, generate_weight_by_shapelet, make_path
import pandas as pd
...
class dbg_dataset(DGLDataset):
    def __init__(self, data_path, k, shapelet_info, data_save_path,window_size=40,reload=False):
        ...
        super(dbg_dataset, self).__init__(name='dbg_dataset', save_dir=self.data_save_path)
        print('***Loading Data ***')
    ...
    def process(self):
        ...
        for i, (lab, seq) in enumerate(tzip(self.labels, rna_seq, desc='generate graphs')):
            graph = de_Bruijn_graph(seq, self.k)
    ...
    def save(self):
        ...
        graph_info = {'graphs': self.graph, 'labels': self.labels, 'shape_weight': self.shape_weight,
                      'shape_freq': self.shape_freq}

        make_path(self.data_save_path)
        with open(f'{self.data_save_path}/{self.data_path.split("/")[-1].split(".")[0]}_k{self.k}_dbg.pkl', 'wb') as f:
            pickle.dump(graph_info, f)
        f.close()

    def load(self):
        tqdm.write('==========load processed data============')
        with open(f'{self.data_save_path}/{self.data_path.split("/")[-1].split(".")[0]}_k{self.k}_dbg.pkl',
                  'rb') as f:
            graph_info = pickle.load(f)
            self.graph = graph_info['graphs']
    ...
    def has_cache(self):
        return not self.reload and os.path.exists(
            f'{self.data_save_path}/{self.data_path.split("/")[-1].split(".")[0]}_k{self.k}_dbg.pkl')
